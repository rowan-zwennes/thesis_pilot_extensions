...
## 2. Codebase Configuration in the Virtual Machine

Navigate to the `PILOT/` project directory cloned during the initial setup. The following modifications must be made to use the code from this study. Assume the new files are provided in the supplementary materials.

1. **Replace Core Files:** Overwrite the original files in the `PILOT/` directory with our modified versions:
   - `PILOT.py`
   - `Tree.py`
   - `download_data.py`
   - `benchmark_config.py`

2. **Add New Files:** Add the following new files to the `PILOT/` directory. The original `benchmark.py` and `benchmark_util.py` can remain but will not be used.
   - `benchmark_new.py`
...
5. **Transfer Data to VM:** Create a new directory named `Data_folder_thesis` inside the `PILOT/` project directory on your virtual machine. Manually transfer all `.csv` files generated by the R script into this new folder.
6. **Execute Python Preprocessing:** In the virtual machine's terminal, from within the `PILOT/` directory, execute the following command. This script will load the `.csv` files and save them as preprocessed `.pkl` files for efficient loading during the main experiment.
   ```bash
...
1. **Locate Python Output:** After running a Python experiment, navigate to the output directory (`Output/<experiment_name>/`). This directory contains two crucial subdirectories:
